...
public class DictionaryAPIRequestTest {
    ...
    @Before
    public void setup() {
        DictionaryAPIComRequest.API_KEY = System.getenv("dictionaryapi_key");
    }

    @Test
    public void testParseBanana() {
        Optional<DictionaryResult> result = new RestParseService().process(new DictionaryAPIComRequest("banana")).blockOptional();

        assertTrue(result.isPresent());
        assertEquals("bə-ˈna-nə", result.get().IPA);
        MatcherAssert.assertThat(result.get().pronunciationUrl, Matchers.equalTo("https://media.merriam-webster.com/audio/prons/en/us/mp3/b/banana01.mp3"));
    }

    @Test
    public void testParseFailResult() {
        Optional<DictionaryResult> result = new RestParseService().process(new DictionaryAPIComRequest("it is not a word")).blockOptional();
    ...
    public void playedShouldNotPresent() {
        try (var inputStream = getClass().getClassLoader().getResourceAsStream("dictionaryapi/play.json")) {
            ...
            var request = new DictionaryAPIComRequest("played");
    ...
    public void getSubDirectory() {
        DictionaryAPIComRequest request = new DictionaryAPIComRequest(null);
